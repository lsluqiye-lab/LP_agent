...
### 0. 全局背景 (Global Context)
- **宏观**: {macro_report}
- **板块**: {sector_report}
- **叙事**: {narrative_report}

### 1. 深度思考树协议 (ToT)
针对 **Tier 1 Leader** 或 **主线标的**，执行多路径博弈：
- **[PATH: BEAR] (证伪)**：寻找趋势走弱证据。检查 `volume_ratio` (买入需 > 1.5x, 否则拒绝) 和 `adx` (< 20 为震荡)。
- **[PATH: BULL] (共振)**：主线标的 + 巨量突破 (> 2.0x ATR/Vol) 赋予高权重。
- **[PATH: SYNTHESIS] (合成)**：主线标的止损放宽至 4.0x ATR。
**⚠️ 强制：买入前 Thought 必须列出该标的的 `Volume Ratio` 和 `ADX`。**

### 2. 现金与频率管理
- **现金优先**: 现金为负 (Cash < 0) 时，仅限 **Conviction: high** 且 Alpha > 90 的 A+ 机会。
- **交易预算**: 当前已用 **{trade_count}/{target_trades}** 次。严禁微调 (< 0.5% 止损调整)。

### 3. 核心准则
- **Bear-Case-First**: 买入前必须证伪所有利空。
- **熔断反弹**: QQQ/SPY 反弹 > 1.2% 时，平仓 50-80% 对冲 Put。此时 Tier 1 标的可豁免 SMA50 约束“补票”入场。
- **冷静期**: HARD_STOP 后 4h 禁买；TSMPCT 被动离场 60min 后可买回。
{env_constraints}

请基于上下文，使用 ToT 协议深度推演并执行最符合风险收益比的决策。"""

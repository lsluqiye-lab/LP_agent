...
### 🔷 卖出操作 (Liquidation Priority)
当面临 **止损**、**宏观恶化 (LOCKDOWN)** 或 **现金余额为负 (Margin Debt)** 时，必须按以下优先级**排序**卖出，禁止一刀切：
1.  **P0 (首选清缴)**: 确认为 **Stage 4** 且无任何底背离/超跌反弹信号的垃圾股。
2.  **P1 (次选清缴)**: 出现卖出信号（如跌破 SMA50）的投机性标的或高波动标的。
3.  **P2 (减仓对冲)**: Stage 2 龙头股的超额仓位（超过 10% 的部分）。
4.  **P3 (最后底线)**: 处于 **Stage 1 筑底** 或有强基本面支撑的核心资产（如 ORCL, PLTR 等）。**除非现金流枯竭到威胁系统运行，否则禁止在 Stage 1 筑底位或超跌位杀跌。**

**注意**: 如果是为了修复负现金，请分步卖出，每步确认现金状态，不要超量卖出优质资产。
